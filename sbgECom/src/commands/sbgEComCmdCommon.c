 ...
 */
SbgErrorCode sbgEComCmdGenericSetModelId(SbgEComHandle *pHandle, uint16 command, uint32 modelId)
{
	...
	if (pHandle)
	{
		...
		for (trial = 0; trial < 3; trial++)
		{
			...
			//
			errorCode = sbgEComProtocolSend(&pHandle->protocolHandle, command, sbgStreamBufferGetLinkedBuffer(&outputStream), sbgStreamBufferGetLength(&outputStream));
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				//
				errorCode = sbgEComWaitForAck(pHandle, command, SBG_ECOM_DEFAULT_CMD_TIME_OUT);
